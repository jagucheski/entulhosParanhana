package br.com.entulhosParanhana.uteis;
...
public class DateUtils {
	...
	public static String getDataPedido(Date date) {
		String retorno = "";
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
			retorno = (new StringBuilder(String.valueOf(retorno))).append(sdf.format(date)).toString();
	...
	/**Retorna o dia atual Adicionando o número de dias conforme prazo do produto**/
	public static Date getDataPrevistaRetirada(int prazoDias) {
		Date dataprevistaRetirada = new Date();
		try {
			Calendar c = Calendar.getInstance();
			c.setTime(dataprevistaRetirada);
			c.add(Calendar.DATE, + prazoDias);
			dataprevistaRetirada = c.getTime();
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}	
